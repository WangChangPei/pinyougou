...
app.controller('goodsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,goodsService){	
	...
	$scope.findOne=function(){	
		...
		//通过商品ID 查询 所有三个张表的对象 包装对象
		goodsService.findOne(id).success(
			function(response){
				//$scope.entity = {goods:{},goodsDesc:{introduction:"商品介绍",itemImages:[{},{},{},...]},itemList:[{},{},{}]};
	...
	$scope.uploadFile = function(){
		...
		uploadService.uploadFile().success(function(response){
			if(response.flag){
				...
				alert(response.message); //<img src="{{image_entity.url}}"
			}
		});
	}
	...
	$scope.$watch("entity.goods.category3Id",function(newValue,oldValue){
		itemCatService.findOne(newValue).success(function(response){//ItemCat
	...
	$scope.$watch("entity.goods.typeTemplateId",function(newValue,oldValue){
		...
		typeTemplateService.findOne(newValue).success(function(response){
			$scope.typeTemplate = response;//TypeTemplate
			// 将品牌的字符串数据转成JSON
